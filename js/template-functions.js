...
$( document ).ready( function(){
	...
	*/
	...
	var navvy = '.navigation';
	var stickyLinkPadding = 'stickyLinkPadding';
	...
	var templateFunctions = {
		...
		siteHeader: function(){
			...
			var onPageScroll = function( win ){
				...
				// Check device
				if ( $( 'body' ).hasClass( 'mobile' ) || $('body').hasClass('permanentStickyMenu') || winW < breakpoint ) return false;

				// Sticky
				if ( thresholdSticky && $( win ).scrollTop() >= thresholdSticky || thresholdSticky === 0 ) {
					$( header ).addClass( headerSticky );
					$( navvy ).addClass( stickyLinkPadding );
					if ( thresholdHeadIn && thresholdHeadOut) $( header ).addClass( headerInitPosition );
				}else{
					$( header ).removeClass( headerSticky );
					$( navvy ).removeClass( stickyLinkPadding );
				...
				// Shrink Logo
				if ( thresholdBkg && $( win ).scrollTop() >= thresholdBkg ) {
					$( topLogo ).addClass( logoShrink );
					$( topLogo ).removeClass( logo );
				} else {
					$( topLogo ).removeClass( logoShrink );
				}
		...
		scrollToSection: function(){
			...
			$( scrollLink ).each( function(){
				...
				$( this ).on( 'click', function( event ){
					...
					var navHeight = $('.header-inner').height();
					var offset = $( this ).data( 'offset' ) ? $( this ).data( 'offset' ) : -navHeight;
